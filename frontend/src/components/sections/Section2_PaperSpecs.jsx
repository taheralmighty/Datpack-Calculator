...
const Section2 = ({ completion }) => {
  ...
  return (
    <SectionCard id="section-2" title="Paper Specifications" index={1} defaultOpen completion={completion}
      subtitle={masterLength && masterWidth ? `${masterLength}×${masterWidth}in · ${gsm || 0}gsm` : undefined}>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <AnimatedInput label="Master Sheet Length" value={masterLength} onChange={v => setField('masterLength', v)} type="number" unit="in" placeholder="28" data-testid="master-length-input" />
